...
const Alert = ({
  ...
  return (
    <div className={`rounded-lg border ${style.border} ${style.bg} p-4 mb-4 ${className}`} role="alert">
      <div className="flex items-start">
        <div className="flex-shrink-0 mr-3 mt-0.5">
          {style.icon}
        </div>
        <div className="flex-1">
          {title && <h3 className="text-sm font-medium text-vision-text mb-1">{title}</h3>}
          {message && <div className="text-sm text-vision-text-secondary">{message}</div>}
        </div>
        {dismissible && (
          <button
            type="button"
            className="ml-auto -mx-1.5 -my-1.5 bg-transparent text-vision-text-secondary hover:text-vision-text p-1.5 rounded-lg focus:outline-none"
            onClick={handleClose}
            aria-label="Close"
          >
